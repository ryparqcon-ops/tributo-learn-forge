-- =====================================================
-- TRIBUTO LEARN - ESQUEMA COMPLETO Y LIMPIO
-- =====================================================
-- Este archivo resetea completamente la base de datos y crea
-- toda la estructura necesaria para el sistema de educación tributaria
...
ALTER TABLE IF EXISTS public.consultation_sessions DISABLE ROW LEVEL SECURITY;
...
DROP FUNCTION IF EXISTS public.calculate_course_rating(course_id UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_instructor_stats(instructor_id UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_student_progress(student_id UUID) CASCADE;
...
DROP TRIGGER IF EXISTS update_consultation_sessions_updated_at ON public.consultation_sessions;
...
BEGIN
    ...
    IF table_count >= 11 THEN
        RAISE NOTICE '✅ Esquema creado correctamente';
    ELSE
        RAISE NOTICE '❌ Error en la creación del esquema';
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Total de vistas creadas: %', view_count;
    
    IF view_count >= 4 THEN
        RAISE NOTICE '✅ Vistas creadas correctamente';
    ELSE
        RAISE NOTICE '❌ Error en la creación de vistas';
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Total de tablas con RLS habilitado: %', rls_count;
    
    IF rls_count >= 11 THEN
        RAISE NOTICE '✅ RLS configurado correctamente';
    ELSE
        RAISE NOTICE '❌ Error en la configuración de RLS';
    END IF;
END $$;
